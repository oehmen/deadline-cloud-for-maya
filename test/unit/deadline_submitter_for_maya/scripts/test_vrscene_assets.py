...
class TestParseVrsceneFile:
    ...
    def test_extracts_unc_paths(self, tmp_path: Path) -> None:
        """Test that UNC paths (//server/share/...) are not stripped as comments"""
        # GIVEN
        vrscene_file = tmp_path / "scene.vrscene"
        vrscene_file.write_text(
            "// This is a real comment and should be ignored\n"
            "BitmapBuffer diffuse_bitmap {\n"
            '    file="//fileserver01/projects/assets/texture.png";\n'
            "}\n"
            "BitmapBuffer normal_bitmap {\n"
            '    file="//fileserver02/jobs/cgi/normal.exr";\n'
            "}\n"
        )
        ...
        # THEN
        assert len(result) == 2
        assert Path(os.path.normpath("//fileserver01/projects/assets/texture.png")) in result
        assert Path(os.path.normpath("//fileserver02/jobs/cgi/normal.exr")) in result
