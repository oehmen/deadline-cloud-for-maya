...
from .data_classes import default_time_entries
...
@dataclass
class MayaSubmitterSettings(BaseSubmitterSettings):
    """Maya-specific submission settings.

    Extends the DCC-agnostic :class:`BaseSubmitterSettings` with the fields the
    Maya job-template/parameter builders consume directly, so the builders can
    read a single settings object without a native-settings translation step.
    """

    # LayerSelection (not a str): the builders compare this against the enum, so
    # the canonical type flows end-to-end. External callers that never touch it
    # simply inherit the ALL default.
    ...
    # A plain field (unlike the GUI settings' property) defaulting to the same
    # entries the native submitter uses. TimeoutTableEntries is Qt-free.
    timeouts: TimeoutTableEntries = field(default_factory=default_time_entries)
...
class MayaSceneContext:
    ...
    Use create_scene_context() to build an instance.
    """
...
def _get_render_layer_data() -> list[RenderLayerData]:
    # Create a dictionary for the layers, and accumulate data about each layer
    logger.debug("_get_render_layer_data - gathering renderable render layer names")
    ...
    logger.debug("_get_render_layer_data - processing %d layer(s)", len(render_layer_names))
    render_layers: list[RenderLayerData] = []
    with saved_current_render_layer():
        for render_layer_name in render_layer_names:
            ...
            logger.debug("_get_render_layer_data - processing layer %s", display_name)
            ...
            logger.debug("_get_render_layer_data - done processing layer %s", display_name)
...
def create_scene_context() -> MayaSceneContext:
    """Gather scene data and build a MayaSceneContext.

    This performs the expensive Maya queries (render layer switching,
    camera enumeration, etc.) exactly once.

    Returns:
        MayaSceneContext with all scene data populated.
    """
    render_layers = _get_render_layer_data()
...
class MayaSubmitter(BaseSubmitter):
    """BaseSubmitter implementation for Maya render submissions.

    Stateful: the expensive Maya scene scan (render-layer switching, camera
    enumeration) is performed once, lazily, and cached on the instance. Both
    ``get_job_template`` and ``get_parameter_values`` reuse that cached
    :class:`MayaSceneContext`, so a single submission queries the scene once.
    Call :meth:`reset_scene_context` (or construct a new instance) after the
    scene changes.
    """

    def __init__(self, scene_context: Optional[MayaSceneContext] = None) -> None:
        # BaseSubmitter defines no __init__ today (it's a pure ABC), so this is
        # currently a no-op — but call it so we stay correct if the base later
        # initializes shared state.
        super().__init__()
        # Allow injecting a pre-built context (used by the GUI, which builds one
        # for its own camera-list population, and by tests).
        self._scene_context = scene_context

    @property
    def scene_context(self) -> MayaSceneContext:
        """The cached scene context, built on first access."""
        if self._scene_context is None:
            self._scene_context = create_scene_context()
        return self._scene_context

    def reset_scene_context(self) -> None:
        """Drop the cached scene context so the next access rescans the scene."""
        self._scene_context = None

    def get_settings(self) -> MayaSubmitterSettings:
        # Read scene values through the shared Scene/Animation helpers rather than
        # raw maya.cmds, so the unified API and the native submitter (which both
        # rely on these helpers) stay consistent:
        #  - Scene.output_path() already anchors the relative "images" file rule to
        #    the project root (otherwise Job Attachments resolves it against the
        #    submit-time CWD and frames are never uploaded back to the project).
        #  - Animation.frame_list() reads the render globals' start/end (the range
        #    that is actually rendered), matching the native dialog; reading the
        #    timeline via playbackOptions would diverge from it.
        scene_name = Scene.name()
        project_path = Scene.project_path() or ""
        output_path = Scene.output_path() or ""

        return MayaSubmitterSettings(
            job_name=Path(scene_name).name or "Untitled",
            project_path=project_path,
            output_path=output_path,
            frame_list=str(Animation.frame_list()),
            input_filenames=[scene_name] if scene_name else [],
            input_directories=[project_path] if project_path else [],
            output_directories=[output_path] if output_path else [],
        )
    ...
    def get_job_template(
        ...
        maya_settings = cast("MayaSubmitterSettings", settings)
        context = self.scene_context

        prepared = self._prepare_render_layers_for_submission(maya_settings, context)
    ...
    def get_parameter_values(
        ...
        # Narrow to the selected layer FIRST (inside _prepare_…), then derive
        # renderers from the submitted layers only. Computing renderers before the
        # filter would emit parameter values (e.g. the Arnold-specific ones) for
        # renderers used by non-submitted layers, disagreeing with
        # get_job_template (which filters first via the same helper).
        prepared = self._prepare_render_layers_for_submission(maya_settings, context)

        return self._get_parameter_values(
            maya_settings, prepared.renderers, prepared.layers, queue_parameters
        )
    ...
    def get_asset_references(self, settings: BaseSubmitterSettings) -> AssetReferences:
        ...
        Introspects the scene the same way the native GUI submitter does
        (``AssetIntrospector.parse_scene_assets``) so the returned inputs
        include textures, referenced scenes, caches, etc. — not just the scene
        file — matching what the native path uploads. Auto-detected paths are
        split into input files vs input directories (non-existent paths are
        dropped, as they cannot be uploaded).

        Output directories combine the scene-derived ``settings.output_directories``
        with the per-layer output locations gathered in the cached scene context
        (``Scene.get_output_directories`` per layer/camera), so camera/layer
        specific render outputs are recorded like the native submitter does.

        Returns the typed :class:`AssetReferences` per the BaseSubmitter
        contract; callers serialize it with ``.to_dict()`` at the job-bundle
        boundary.

        Note: this performs the full scene walk (the same expensive scan the GUI
        shows a progress bar for), so it is intentionally not folded into the
        cached scene context.
        """
        input_filenames: set[str] = set()
        input_directories: set[str] = set()

        scene_file = Scene.name() or ""
        if scene_file:
            input_filenames.add(scene_file)

        # Auto-detect referenced assets (textures, references, caches, …) and
        # classify each existing path as a file or a directory. Mirrors the
        # native GUI submitter's handling in show_maya_render_submitter.
        for asset_path in AssetIntrospector().parse_scene_assets():
            normalized = os.path.normpath(str(asset_path))
            if not os.path.exists(normalized):
                continue
            if os.path.isdir(normalized):
                input_directories.add(normalized)
            else:
                input_filenames.add(normalized)

        # Outputs: scene-derived defaults plus every render layer's output dirs.
        output_directories: set[str] = set(settings.output_directories)
        for layer in self.scene_context.render_layers:
            output_directories.update(layer.output_directories)
    ...
    def _prepare_render_layers_for_submission(
        ...
        When ``render_layer_selection`` is CURRENT, it narrows to the current
        render layer and raises a user-facing DeadlineOperationError (matching
        the native submitter) if that layer is not renderable, rather than
        letting a downstream ``render_layers[0]`` raise an opaque IndexError.

        Args:
            settings: The Maya submitter settings.
        ...
        # The per-layer parameter names below (Frames/OutputFilePrefix/Image*)
        # are keyed on layer_data.display_name. Duplicate display names would
        # collide on the same job-template parameter, silently dropping one
        # layer's frame range / output prefix / resolution. renderSetup keeps
        # layer names unique in practice, but guard explicitly so a collision
        # fails loudly with an actionable message instead of mis-rendering.
        display_names = [layer.display_name for layer in submit_render_layers]
        duplicate_display_names = sorted(
            {name for name in display_names if display_names.count(name) > 1}
        )
        if duplicate_display_names:
            raise DeadlineOperationError(
                "Render layers must have unique display names to submit, but these "
                f"are duplicated: {', '.join(duplicate_display_names)}."
            )
